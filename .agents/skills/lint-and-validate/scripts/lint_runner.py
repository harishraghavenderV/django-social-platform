...
def detect_project_type(project_path: Path) -> dict:
    ...
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        ...
        ignore_rules = "--ignore=E722,F841,F401,E701,F541,E501"
        if venv_ruff_win.exists():
            ruff_cmd = [str(venv_ruff_win), "check", ".", ignore_rules]
        elif venv_ruff_unix.exists():
            ruff_cmd = [str(venv_ruff_unix), "check", ".", ignore_rules]
        else:
            ruff_cmd = ["ruff", "check", ".", ignore_rules]
