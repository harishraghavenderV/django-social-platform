...
def detect_test_framework(project_path: Path) -> dict:
    ...
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        ...
        if (project_path / "manage.py").exists():
            ...
            venv_python_unix = project_path / "venv" / "bin" / "python"
    ...
    try:
        ...
        if "pytest" in str(cmd):
            ...
            if match:
                result["tests_failed"] = int(match.group(1))
            result["tests_run"] = result["tests_passed"] + result["tests_failed"]
        # Django pattern: "Ran X tests"
        elif "manage.py" in str(cmd):
            import re
            ...
            if match:
                ...
                if "FAILED" in output:
                    ...
                    result["tests_failed"] = fails + errs
                    result["tests_passed"] = total_tests - result["tests_failed"]
                else:
